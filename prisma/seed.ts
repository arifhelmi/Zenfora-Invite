...
async function main() {
  ...
  const categories = await Promise.all([["heritage", "Heritage"], ["sunda", "Sunda"], ["minimal", "Minimal"], ["romantic", "Romantic"], ["islamic", "Islamic"], ["playful", "Playful"], ["corporate", "Corporate"], ["formal", "Formal"]].map(([slug, name]) => prisma.themeCategory.upsert({ where: { slug }, update: { name }, create: { slug, name } })));
  ...
  for (const [slug, name, style, colors, isPremium, isFeatured, description, colorTokens] of themes) {
    ...
    const asset = slug === "sunda-parahyangan"
      ...
      : { key: "ornament", path: `/themes/${slug}/ornament.svg`, source: "Zenvora original SVG", license: "Proprietary original, internal use" };
    await prisma.themeAsset.upsert({ where: { themeId_key: { themeId: theme.id, key: asset.key } }, update: asset, create: { themeId: theme.id, ...asset } });
  ...
  const packageSeeds: { slug: string; name: string; description: string; price: number; guestLimit: number; activeDays: number; features: string[] }[] = [
    { slug: "free-demo", name: "Free Demo", description: "Mulai mencoba tanpa risiko.", price: 0, guestLimit: 20, activeDays: 14, features: ["Satu tema gratis", "Maksimum 20 tamu", "Watermark Zenvora Invite"] },
    { slug: "basic", name: "Basic", description: "Semua yang diperlukan untuk sebuah acara keluarga.", price: 99000, guestLimit: 300, activeDays: 180, features: ["Tema basic", "RSVP", "Galeri", "Ucapan", "Aktif enam bulan"] },
    { slug: "premium", name: "Premium", description: "Kapasitas dan fitur lengkap untuk momen besar.", price: 199000, guestLimit: 1000, activeDays: 365, features: ["Semua tema", "QR check-in", "Analytics", "Kado digital", "Tanpa watermark"] }
  ];
