...
export function InvitationRenderer({ theme, event, guest, isPreview = false }: { theme: Theme & { versions: ThemeVersion[] }; event: RenderEvent; guest?: { token: string; name: string; seats: number }; isPreview?: boolean }) {
  ...
  return <div className={`invite ${themeStyles[theme.slug] ?? "minimal"}`} data-sunda-template={isSunda ? "true" : undefined} data-jawa-template={isJawa ? "true" : undefined} style={{ "--invite-background": tokens.background, "--invite-surface": tokens.surface, "--invite-primary": tokens.primary, "--invite-accent": tokens.accent, "--invite-text": tokens.text, "--invite-radius": manifest.tokens.radius.card } as React.CSSProperties}>
    ...
    <section {...revealProps} className={panelClass("cover", "cover section")}>{isSunda && <><span aria-hidden="true" className="sunda-cover-wayang-motion sunda-cover-wayang-motion-left"><span className="sunda-cover-wayang sunda-cover-wayang-left" /></span><span aria-hidden="true" className="sunda-cover-wayang-motion sunda-cover-wayang-motion-right"><span className="sunda-cover-wayang sunda-cover-wayang-right" /></span></>}{isJawa && <><span aria-hidden="true" className="jawa-cover-wayang-motion jawa-cover-wayang-left"><span className="jawa-cover-wayang jawa-cover-wayang-inward-left" /></span><span aria-hidden="true" className="jawa-cover-wayang-motion jawa-cover-wayang-right"><span className="jawa-cover-wayang jawa-cover-wayang-inward-right" /></span></>}<div className="content">{ornament && <div className="ornament" aria-hidden="true">{ornament}</div>}<p className="text-sm uppercase tracking-[.22em]">{isSunda ? "Undangan Pernikahan" : `Undangan ${theme.style}`}</p><h1 className="mt-6 font-serif text-5xl leading-tight sm:text-6xl">{event.title}</h1>{guest && <p className="mt-6 text-lg">Kepada Yth. <strong>{guest.name}</strong></p>}<p className="mt-8 text-lg">{event.startsAt ? formatDate(event.startsAt) : "Tanggal akan diumumkan"}</p><a className="mt-8 inline-flex rounded-lg border border-current px-4 py-2 text-sm font-bold" href="#detail">Lihat detail acara</a></div></section>
