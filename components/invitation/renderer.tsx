...
export function InvitationRenderer({ theme, event, guest, isPreview = false }: { theme: Theme & { versions: ThemeVersion[] }; event: RenderEvent; guest?: { token: string; name: string; seats: number }; isPreview?: boolean }) {
  ...
  const isCultural = isSunda || isJawa;
  const panelClass = (name: string, className = "section") => `${className}${isSunda ? ` sunda-panel sunda-${name}` : ""}${isJawa ? ` jawa-panel jawa-${name}` : ""}`;
  const revealProps = isCultural ? { "data-invitation-reveal": "true", "data-sunda-reveal": isSunda ? "true" : undefined } : {};
  const ornament = isJawa ? "✦ Jawa ✦" : "✦ Sunda ✦";
  ...
  const renderSection = (key: string) => {
    ...
    if (key === "gallery") return <section {...revealProps} className={panelClass("gallery")}><div className="content"><p className="eyebrow text-center">Galeri</p><div className={`mt-6 grid grid-cols-2 gap-3${isSunda ? " sunda-gallery" : isJawa ? " jawa-gallery" : ""}`}><div className="aspect-square rounded-xl bg-[color:color-mix(in_srgb,var(--invite-accent),transparent_70%)]" /><div className="aspect-square rounded-xl bg-[color:color-mix(in_srgb,var(--invite-primary),transparent_76%)]" /><div className="col-span-2 aspect-[2/1] rounded-xl bg-[color:color-mix(in_srgb,var(--invite-surface),var(--invite-accent)_20%)]" /></div></div></section>;
    ...
    if (key === "closing") return <section {...revealProps} className={panelClass("closing", "section text-center")}><div className="content"><div className="ornament">{ornament}</div><p className="text-xl">Terima kasih atas doa dan kehadiran Anda.</p><p className="mt-6 text-sm opacity-65">Dibuat dengan Zenvora Invite</p></div></section>;
    return null;
  };

  return <div className={`invite ${themeStyles[theme.slug] ?? "minimal"}`} data-sunda-template={isSunda ? "true" : undefined} data-jawa-template={isJawa ? "true" : undefined} style={{ "--invite-background": tokens.background, "--invite-surface": tokens.surface, "--invite-primary": tokens.primary, "--invite-accent": tokens.accent, "--invite-text": tokens.text, "--invite-radius": manifest.tokens.radius.card } as React.CSSProperties}>
    {isCultural && <InvitationScrollEffects />}
    <section {...revealProps} className={panelClass("cover", "cover section")}>{isSunda && <><span aria-hidden="true" className="sunda-cover-wayang-motion sunda-cover-wayang-motion-left"><span className="sunda-cover-wayang sunda-cover-wayang-left" /></span><span aria-hidden="true" className="sunda-cover-wayang-motion sunda-cover-wayang-motion-right"><span className="sunda-cover-wayang sunda-cover-wayang-right" /></span></>}{isJawa && <><span aria-hidden="true" className="jawa-cover-wayang-motion jawa-cover-wayang-left"><span className="jawa-cover-wayang jawa-cover-wayang-inward-left" /></span><span aria-hidden="true" className="jawa-cover-wayang-motion jawa-cover-wayang-right"><span className="jawa-cover-wayang jawa-cover-wayang-inward-right" /></span></>}<div className="content"><div className="ornament" aria-hidden="true">{ornament}</div><p className="text-sm uppercase tracking-[.22em]">Undangan {theme.style}</p><h1 className="mt-6 font-serif text-5xl leading-tight sm:text-6xl">{event.title}</h1>{guest && <p className="mt-6 text-lg">Kepada Yth. <strong>{guest.name}</strong></p>}<p className="mt-8 text-lg">{event.startsAt ? formatDate(event.startsAt) : "Tanggal akan diumumkan"}</p><a className="mt-8 inline-flex rounded-lg border border-current px-4 py-2 text-sm font-bold" href="#detail">Lihat detail acara</a></div></section>
